package yliasdiscordbothx.config;

// Rename this file to AuthDetails before compiling

class AuthDetails_example {
    // Discord
    public static inline var DISCORD_TOKEN = '';
    public static inline var DISCORD_EMAIL = '';
    public static inline var DISCORD_PASSWORD = '';
    public static inline var OWNER_ID = 0;

    // Database
    public static inline var DB_HOST = '';
    public static inline var DB_USER = '';
    public static inline var DB_PASSWORD = '';
    public static inline var DB_NAME = '';

    // Google API
    public static inline var GOOGLE_API_KEY = '';
    public static inline var GOOGLE_CSE_ID = '';

    // Imgflip API
    public static inline var IMGFLIP_USERNAME = '';
    public static inline var IMGFLIP_PASSWORD = '';

    // e621 API
    public static inline var E621_LOGIN = '';
    public static inline var E621_PASSWORD_HASH = '';

    // Derpibooru API
    public var DERPIBOORU_KEY = '';

    // Getty API
    public static inline var GETTY_KEY = '';
    public static inline var GETTY_SECRET = '';

    // Imgur
    public var IMGUR_CLIENT_ID = '';
    public var IMGUR_CLIENT_SECRET = '';

    public function new() {}
}
